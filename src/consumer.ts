...
const consumer2: Consumer = kafka.consumer({ groupId: 'notification-group' });
...
const runConsumer = async (): Promise<void> => {
  ...
  await consumer2.subscribe({ topic: 'email-topic' });
  await consumer2.subscribe({ topic: 'sms-topic' });
